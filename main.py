...
menuOpt = allMenus[
    "mainmenu"
]  # menuOpt will store all menu options of the current menu and will be updated on any menu change
...
previousMenus = (
    []
)  # using an array here in case of future sub-submenus, essentially storing the menu pathway so we can backtrack
...
availableColours = [
    ...
    "Cyan",
    "Purple",
]  # we'll cycle through this when changing colours
currentColourIndex = int(
    availableColours.index(optionsJson["textcolour"].capitalize())
)  # locating the saved colour stored in options.json and loading the index
...
titles = [
    "TXTEXE",
    "TEXET",
]  # store the names of the title since we WILL NOT SEARCH FOR ASCII ART JAMES
titleChoice = int(
    titles.index(optionsJson["title"].upper())
)  # loading saved title choice from options.json


def caseInput(key):  # handling any input recieved from listener
    if (
        rootWindow.focus_displayof() and not rootWindow.focus_displayof() == userEntry
    ):  # checking that the window is 1. in focus and 2. the user isn't typing in any entry box
        global currentIndex
        ...
        match (key):
            ...
            case Key.left:
                if inMenu:
                    ...
                    if (
                        str(currentMenu.lower() + ":" + optionToSearch.lower() + ":")
                        in allMenus["arrowkeymenu"]
                    ):
            ...
            case Key.enter:
                if inMenu:
                    selectIndex(
                        currentIndex
                    )  # selects the index if the player is in a menu
                else:
                    submitText()  # attempts to submit text
    elif (
        key == Key.esc and rootWindow.focus_displayof() == userEntry
    ):  # this removes focus from the entryWidget if esc is pressed
...
def changeTextSpeed(direction):
    ...
    if direction.lower() == "left":
        ...
        if (
            textSpeedMult < 0.5
        ):  # ensures that the new text speed isn't below the minimum speed
            textSpeedMult = 0.5
    else:
    ...
    textSpeedMult = round(
        textSpeedMult, 1
    )  # fixes the floating point error caused by this
    menuOpt[currentIndex] = (
        menuOpt[currentIndex].split(":")[0] + ":" + " < " + str(textSpeedMult) + " >"
    )  # updating the label at currentIndex to reflect the new chosen option
    allMenus[currentMenu][currentIndex] = (
        menuOpt[currentIndex].split(":")[0] + ":" + " < " + str(textSpeedMult) + " >"
    )  # Updating the value in allMenus to save the updated value when the menu is exited
...
def textColourSwap(direction):
    ...
    if direction.lower() == "left":
        ...
        if currentColourIndex < 0:  # range check - min
            currentColourIndex = (
                len(availableColours) - 1
            )  # sets it to the final index of availableColours if the index becomes too low
    else:
        currentColourIndex += 1
        if currentColourIndex > len(availableColours) - 1:  # range check - max
            currentColourIndex = 0
    currentColour = availableColours[
        currentColourIndex
    ].lower()  # updating currentColour
    ...
    allMenus[currentMenu][currentIndex] = (
        menuOpt[currentIndex].split(":")[0] + ":" + newChoice
    )  # making sure that the menu choice is saved across menus
    redrawMenu(menu)  # update display
...
def menuDown():
    ...
    global currentIndex
    if (
        currentIndex + 1 <= len(menuOpt) - 1
    ):  # checking if the user is not at the end of the menu
        menuOpt[currentIndex + 1] = (
            "> " + menuOpt[currentIndex + 1]
        )  # same as menuUp but in opposite direction
        menuOpt[currentIndex] = menuOpt[currentIndex].replace("> ", "")
        currentIndex += 1
        redrawMenu(menu)  # update display
...
def menuBack(parentMenu, menuWidget):
    if len(parentMenu) > 0:  # checking if there is a menu to return to
        ...
        if currentMenu == "options":  # save options if exiting options menu
            saveOptions()
        allMenus[currentMenu][currentIndex] = allMenus[currentMenu][
            currentIndex
        ].replace(
            "> ", ""
        ...
        for option in parentMenu:
            menuOpt.append(
                option
            )  # repopulating the menuOpt list with the parentMenu's options
...
def loadOptions():
    ...
    for item in range(0, len(allMenus["options"])):
        match (
            count
        ):  # TODO: please change this to be easily modifiable by potentially dynamically changing variables from a list
            case 0:
                currentItem = "false"
            case 1:
        ...
        optionToSearch = optionToSearch.replace(
            "> ", ""
        )  # remove the cursor if it's there
        if (
            optionToSearch in allMenus["arrowkeymenu"]
            or optionToSearch in annoyingBugFix
        ):
            allMenus["options"][item] += (
                " < " + str(currentItem) + " >"
            )  # arrowkeymenu: load the label with the arrowkeymenu structure: OptionName: < {savedOption} >
        else:
            allMenus["options"][item] += " " + str(
                currentItem
            )  # not arrowkeymenu: load the label as just optionName: {savedOption}
        count += 1
...
def saveOptions():  # TODO: if the player is currently in the options menu and chooses to close the game, prompt them that they have unsaved changes
    ...
    for option in range(0, len(allMenus["options"])):
        optionToSearch = (
            "options:" + allMenus["options"][option].lower().split(": ")[0] + ":"
        )
        optionToSearch = optionToSearch.replace(
            "> ", ""
        )  # remove the cursor if it's there
        ...
        if (
            ...
            optionData = (
                allMenus["options"][option].split("< ")[1].lower()
            )  # arrowkeymenu: split the string containing the optionName and optionChoice at "< " in optionName: < optionChoice >, selecting the final index
            print(optionData)
            ...
            optionData = (
                allMenus["options"][option].split(": ")[1].lower()
            )  # not arrowkeymenu: just split it at ": " in optionName: optionChoice, leaving just optionChoice
        ...
        optionName = (
            allMenus["options"][option].split(":")[0].lower()
        )  # isolating optionName
...
def enterSubMenu(menuWidget, option):
    ...
    allMenus[currentMenu][currentIndex] = allMenus[currentMenu][currentIndex].replace(
        "> ", ""
    )  # remove the cursor at the current position
    previousMenus.insert(
        0, currentMenu
    )  # add the current menu to previousMenus at position 0 to enable backtracking later
...
def selectIndex(currentIndex):
    ...
    if isSubMenuTrigger:
        ...
        match (
            currentMenu
        ):  # the match case here is to find the current menu and choose the relevant option accordingly
            case "mainmenu":
                match (option.lower()):
                    case "start":
                        listener.stop()
                        inMenu = False
                        userEntry.place(
                            x=20, y=rootWindow.winfo_height() - 20
                        )  # make the userEntry visible
                        ...
                        ldScreen.loadIn(
                            ...
                            chrName,
                            textSpeedMult,
                            textColour=currentColour,
                            widgetConserve=widgetConserve,
                        )  # loading screen
                        character = CC.welcome(
                            rootWindow=rootWindow,
                            entryObj=userEntry,
                            textSpeedMult=textSpeedMult,
                            textColour=currentColour,
                            userEntry=userEntry,
            ...
            case "options":
                if "title:" in option.lower():
                    ...
                    allMenus["options"][1] = allMenus["options"][1].replace(
                        titles[titleChoice], ""
                    )  # replacing the title choice with the next one
                    titleChoice += 1
                    if titleChoice > len(arts) - 1:  # range check
                        titleChoice = 0
                    allMenus["options"][1] += titles[
                        titleChoice
                    ]  # update allmenus to represent new value
                    redrawMenu(menu)  # update display
...
title = Text(
    ...
    borderwidth=0,
    bg="black",
    fg=availableColours[currentColourIndex],
...
entryIndicator = Label(
    rootWindow, font=("Courier", 15), fg=currentColour, bg="black", text="> "
)
userEntry = Entry(
    rootWindow,
    width=150,
...
widgetConserve = [
    ...
    userEntry,
    focusRemove,
]  # any widget which shouldn't be deleted goes in here
